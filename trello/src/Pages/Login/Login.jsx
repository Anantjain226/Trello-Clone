import React, {useState} from 'react'
import { useHistory } from 'react-router-dom'
import styles from './style.module.css'


export const Login = () => {
    const [email, setEmail] = useState("")
    const [pass, setPass] = useState("")
    const history = useHistory()

    const handleClick = () => {
        if(email == "anantjain226@gmail.com" && pass == "admin"){
            history.push("/Boards")
        }
        
    }

    return (
        <div className = {styles.login} >
            <img  src = "https://www.atlassian.com/dam/jcr:c135e4c1-d61c-4299-a536-c710530e2e64/Trello-icon-blue@2x.png"></img>
            <div>
                <div>
                    <p>Log in to Trello</p>
                    <input 
                        type = "email" 
                        placeholder = "Enter Email" 
                        value = {email} 
                        onChange = {(e) => setEmail(e.target.value)}
                    /> 
                    <input type = "password" placeholder = "Enter password" value = {pass} onChange = {(e) => setPass(e.target.value)} ></input>
                    <button onClick = {handleClick} > Log in </button>
                    <p> OR </p>
                    <div>
                        <img src= "data:image/png;base64," ></img>
                        <p> Continue with Google</p>
                    </div>
                    <div>
                        <img src= "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT2RfKVAKNjDKphSxswsD2MKtfiA_nkI8wPPQ&usqp=CAU" ></img>
                        <p> Continue with Microsoft</p>
                     </div>
                    <div>
                        <img src= "https://upload.wikimedia.org/wikipedia/commons/f/fa/Apple_logo_black.svg" ></img>
                        <p> Continue with Apple</p>
                    </div>
                    <p>Log in with SSO</p>
                    <hr></hr>
                    <p>Can't log in? Sign up for an account</p>
                </div>
            </div>
        </div>
    )
}
